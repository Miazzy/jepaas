...
(function(root, factory) {
	...
	$.fn.powerTip = function(opts, arg) {
		...
		//linzhichao 2017-5-8 自动显示
		if(options.autoShow){
			$.powerTip.show(targetElements);
		}
		return targetElements;
	};
	...
	function TooltipController(options) {
		...
		function hideTip(element) {
			if(!element){return;}//2017-5-4 linzhichao
			// reset session
		...
		function closeDesyncedTip() {
			...
			//linzhichao 2017-5-8 判断activeHover是否为空
			if (session.activeHover && session.isTipOpen && !session.isClosing && !session.delayInProgress && ($.inArray('mouseleave', options.closeEvents) > -1 || $.inArray('mouseout', options.closeEvents) > -1 || $.inArray('blur', options.closeEvents) > -1 || $.inArray('focusout', options.closeEvents) > -1)) {
