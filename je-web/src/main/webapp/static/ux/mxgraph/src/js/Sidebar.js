...
function Sidebar(editorUi, container)
{
	...
	this.editorEntity = editorUi.editorEntity;
 ...
 */
Sidebar.prototype.thumbWidth = 40;

/**
 * Specifies the height of the thumbnails.
 */
Sidebar.prototype.thumbHeight = 40;
 ...
 */
Sidebar.prototype.sidebarTitleSize = 12;
 ...
 * Adds the general palette to the sidebar.
 */
Sidebar.prototype.addGeneralPalette = function(dom)
{
	var lineTags = 'line lines connector connectors connection connections arrow arrows ';
	
	var fns = [
		this.createVertexTemplateEntry('swimlane;', 200, 300, '', '容器', null, null, 'container swimlane lane pool'),
		this.createVertexTemplateEntry('swimlane;horizontal=0', 300, 200, '', '容器', null, null, 'container swimlane lane pool'),
...
//			'Textbox', null, null, 'text textbox textarea'),
	...
	this.addPaletteFunctions(dom, fns);
};

/**
 * Adds the general palette to the sidebar.
 */
Sidebar.prototype.addBasicPalette = function(dom)
{
	this.addStencilPalette(dom, STENCIL_PATH + '/basic.xml',
...
Sidebar.prototype.addArrowsPalette = function(dom)
{
	this.addStencilPalette(dom,STENCIL_PATH + '/arrows.xml',
		';whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#000000;strokeWidth=1');
};
Sidebar.prototype.addFlowchartPalette = function(dom)
{
	this.addStencilPalette(dom, STENCIL_PATH + '/flowchart.xml',
		';whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#000000;strokeWidth=1');
};
/**
 * Adds the general palette to the sidebar.
...
//	node.style.top = this.thumbBorder + 'px';
	...
	if (this.sidebarTitles && title != null && showTitle)
	{
	...
	return mxUtils.bind(this, function(graph, evt, target, x, y)
	{
		if (graph.isEnabled())
		{
			...
			if (cells.length > 0)
			{
				graph.stopEditing();
				//放下之前操作
				if(this.editorEntity.beforeCellDrop(cells[0]) == false){
					return;
				}
				...
				if (!graph.isCellLocked(target || graph.getDefaultParent()))
				{
					...
					{
						//放下操作
						this.editorEntity.cellDrop(select[0]);
						graph.scrollCellToVisible(select[0]);
						graph.setSelectionCells(select);
						//放下之后操作
						this.editorEntity.endCellDrop(select[0]);
					}
				}
			}
 ...
 * @return {}
 */
Sidebar.prototype.formatValue = function(value,type){
...
{
	value = this.formatValue(value,'node');
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
	cells[0].vertex = true;
	this.editorEntity.buildCell(cells[0]);//格式化节点  linzhichao
...
Sidebar.prototype.createEdgeTemplate = function(style, width, height, value, title, showLabel, allowCellsInserted)
{
	...
	cell.geometry.relative = true;
	cell.edge = true;
	this.editorEntity.buildCell(cell);//格式化节点  linzhichao
...
{	
	return this.createItem(cells, title, showLabel, false, width, height, allowCellsInserted);
};

/**
 * Adds the given palette.
 */
Sidebar.prototype.addPaletteFunctions = function(content, fns)
{
 ...
 */
Sidebar.prototype.addStencilPalette = function(dom, stencilFile, style, ignore, onInit, scale, tags, customFns)
{
	...
	mxStencilRegistry.loadStencilSet(stencilFile, mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
	{
		if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
		{
			...
			me.addPaletteFunctions(dom, [this.createVertexTemplateEntry('shape=' + packageName + stencilName.toLowerCase() + style,
